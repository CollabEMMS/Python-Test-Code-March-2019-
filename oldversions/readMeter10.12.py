...
def excelSheet(name, osStartTime, ekmPower, emmsPower, ekmEnergyChange, emmsEnergyChange):
    ...
    data.append([osElapsedTime,ekmPower,emmsPower,ekmEnergyChange,emmsEnergyChange])
...
def main():
    ...
    if (delay == ''):
        delay = 1  #For testing purposes, the default delay will be 1 second
        print("1") #Please change this after this code is finished
    else:
        delay = int(delay)
    ...
    ekm = serial.Serial(
        ...
        xonxoff=0,
        timeout=delay       # time before reading
    )
    emms = serial.Serial(
    ...
    # First Contact (gets the original timestamp and energy info)
    ...
    data = [["File Name",name],["Date",date],["Test Info",testInfo],["Directory",filePath],[],["System TIMESTAMP", "TIMESTAMP", "EKMPOWER", "EMMS POWER", "EKM Energy Used", "EMMS Energy Used"]]
...
# Check if file already exists (File - #.csv)
